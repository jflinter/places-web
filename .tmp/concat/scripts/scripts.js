...
/**
 * @ngdoc function
 * @name placesWebApp.controller:MainCtrl
 * @description
 * # MainCtrl
 * Controller of the placesWebApp
 */
angular.module('placesWebApp').controller('MainCtrl', [
  '$scope',
  function ($scope) {
    $scope.awesomeThings = [
      'HTML5 Boilerplate',
 ...
 * Controller of the placesWebApp
 */
angular.module('placesWebApp').controller('MapsCtrl', [
  '$scope',
  '$firebase',
  '$routeParams',
  'leafletData',
  'leafletEvents',
  function ($scope, $firebase, $routeParams, leafletData, leafletEvents) {
    var markerCache = {};
    $scope.title = 'Loading...';
    $scope.markers = [];
    ...
    $scope.$on('leafletDirectiveMap.popupopen', function (event, args) {
      ...
      leafletData.getMap().then(function (map) {
        ...
        photoRef.once('value', function (photoSnap) {
          var imageData = _.map(photoSnap.val(), function (photo) {
              return photo.image;
            })[0];
          if (imageData) {
        ...
        var diff = 70;
        // if (diff > 0) {
        var targetPoint = map.project(latlng, zoom).subtract([
            0,
            diff
          ]);
      ...
      if (mapId) {
        ...
        mapRef.once('value', function (snapshot) {
          ...
          var markers = _.map(places, function (place) {
              ...
              var content = '<div class=\'marker-content\' data-placeid=\'' + place.id + '\'>' + '<div class=\'marker-title\'>' + firstLine + '</div>' + rest + '</div>';
              var marker = {
                  lat: place.latitude,
                  lng: place.longitude,
                  message: content,
